...
class SimpleReliableCamera {
  constructor(options = {}) {
    ...
    this.minRequestInterval = 280; // Minimum ms between server requests
    this.watchdogTimer = null;
    this.watchdogInterval = 10000; // 10 seconds
    this.baseCaptureInterval = 180;
    this.minCaptureInterval = 150;
    ...
    this.faceTracker = {
      ...
      minProcessInterval: 250, // Minimum 250ms between requests for same faces
  ...
  applyPerformanceProfile() {
    if (this.deviceTier === "low") {
      ...
      this.faceTracker.minProcessInterval = 700;
    ...
    if (this.deviceTier === "mid") {
      ...
      this.faceTracker.minProcessInterval = 600;
      this.faceTracker.newFaceProcessInterval = 340;
      this.faceTracker.maxStableFrames = 6;
    }

    if (this.deviceTier === "high") {
      this.faceTracker.minProcessInterval = 250;
      this.faceTracker.maxStableFrames = 8;
    }
  }
  ...
  shouldProcessFrame() {
    ...
    if (now - tracker.lastProcessTime > 10000) {
      // 10 seconds
    ...
    if (now - tracker.lastSignificantChange < 5000) {
      // Within 5 seconds of change
  ...
  updateFaceTracker(faces = []) {
    ...
    if (hasSignificantChange) {
      ...
      console.log(
        `📊 Stable face count: ${tracker.stableFaceCount}/${tracker.maxStableFrames}`,
      );
    }
  ...
  async start() {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "Camera access requires a secure context (HTTPS, localhost, or loopback)",
        );
      }
        ...
        // Method 1: Standard constraints optimized for face detection
        {
          video: {
            ...
            frameRate: prefersLowPowerProfile
              ? { ideal: 10, max: 15, min: 8 }
              : { ideal: 15, max: 20, min: 10 },
            facingMode: "user",
          },
          audio: false,
        },
      ...
      while (playAttempts < maxPlayAttempts) {
        try {
          playAttempts++;
          console.log(
            `▶️ Video play attempt ${playAttempts}/${maxPlayAttempts}`,
          );
          ...
          console.warn(
            `❌ Video play attempt ${playAttempts} failed:`,
            playError,
          );
          ...
          if (playAttempts < maxPlayAttempts) {
            ...
            throw new Error(
              `Video playback failed after ${maxPlayAttempts} attempts: ${playError.message}`,
            );
          }
        }
      }
      ...
      const processingScale = Math.min(
        1,
        this.targetProcessingWidth / videoWidth,
      );
      this.canvas.width = Math.max(
        160,
        Math.round(videoWidth * processingScale),
      );
      this.canvas.height = Math.max(
        120,
        Math.round(videoHeight * processingScale),
      );
  ...
  drawFaceDetectionBox(
    ...
    if (
      faceType === "spoofing_detected" ||
      label.includes("SPOOFING DETECTED")
    ) {
  ...
  async captureAndSendImage() {
    ...
    // Simple throttle: don't start a new request too soon after the last one
    const now = Date.now();
    if (now - this.lastRequestStartTime < this.minRequestInterval) {
      this.scheduleNextCapture(
        this.minRequestInterval - (now - this.lastRequestStartTime),
      );
      return;
    }
    ...
    try {
      ...
      this.adaptiveCaptureInterval = Math.max(
        ...
        Math.min(this.maxCaptureInterval, Math.round(captureDuration * 0.3)),
      );
      ...
      // Handle registration (outside faces_detected check since server may not return it)
      if (this.mode === "registration" && result.capture_count !== undefined) {
        ...
        // Notify UI of progress even if no new face was detected this frame
        this.onFaceDetected({
